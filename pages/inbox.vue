<template>
  <EmailModal
    ...
    :archiveEmail="archiveOpenedEmail"
    :closeModal="closeModal"
  />
  <div class="title">
    <h1 class="title__text">Inbox</h1>
  </div>
    ...
    <div class="email-list__header">
      ...
      <div class="email-list__options-btn-group">
        <div
          ...
          @click="archiveEmails(selectedEmails)"
...
export default {
  setup() {
    ...
    const { inboxEmails } = storeToRefs(useEmailStore());
    ...
    const allSelected = computed(() => {
      return inboxEmails.value.length > 0
        ? inboxEmails.value.length === selectedEmails.value.size
        : false;
    });
    ...
    const archiveEmails = () => {
      selectedEmails.value.forEach((emailId) => {
        emailStore.archiveEmail(emailId);
      });
      selectedEmails.value.clear();
    };
    ...
    const archiveOpenedEmail = () => {
      emailStore.archiveEmail(openedEmail.value.id);
      closeModal();
    };
